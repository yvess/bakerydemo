...
def search(request):
    ...
    site_root = site_root.specific.get_translation_for_request(request) or site_root
    ...
    if search_query:
        if 'elasticsearch' in settings.WAGTAILSEARCH_BACKENDS['default']['BACKEND']:
            ...
            # like this:
            search_results = Page.objects.live().descendant_of(site_root).search(search_query)
        else:
            ...
            search_results = Page.objects.live().descendant_of(site_root).filter(id__in=page_ids)
